...
export default async function handler(req, res) {
  ...
  // Vérif crédits
  if (!user) {
    // Non connecté : on bloque (front gère les crédits localStorage)
    // On laisse passer avec un flag "guest"
  } else if (user.plan !== 'pro' && user.credits <= 0) {
...
Réponds UNIQUEMENT avec le texte final prêt à l'emploi. Aucun commentaire, aucune introduction.`;
  ...
  try {
    ...
    // Décrémenter les crédits si free
    if (user && user.plan !== 'pro') {
      user.credits = Math.max(0, (user.credits || 0) - 1);
    }

    return res.status(200).json({ text, credits: user?.credits ?? null, plan: user?.plan ?? 'guest' });
